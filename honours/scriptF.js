...
function fuelConsunption(event) {
    ...
    if (!depArr || !arrAlt || !airSpd || !fuelCons) {
        ...
        fuelResult.innerHTML=" Fill in at least the first 4 fields!"; 
    ...
    if (!totalFuel || totalFuel === NaN) {
        document.getElementById('fuelResultError').innerHTML ="Insufficient data!";
        document.getElementById('fuelAlertError').style.display="block";
        document.getElementById('fuelAlert').style.display="none";
    } else {
        document.getElementById('fuelResult').innerHTML ="you will use " + totalFuel.toFixed(1).replace('.' , ',') + " " + metrica;
...
function fuelConsunptionVfr (event) {
    ...
    if (!depArr || !airSpd || !fuelCons) {
        ...
        fuelResult.innerHTML=" Fill in at least the first 3 fields!"; 
    } else {
    ...
    if (!totalFuel || totalFuel === NaN) {
        document.getElementById('fuelResultErrorV').innerHTML ="Insufficient data!";
        document.getElementById('fuelAlertErrorV').style.display="block";
        document.getElementById('fuelAlertV').style.display="none";
    } else {
        document.getElementById('fuelResultV').innerHTML ="you will use  " + totalFuel.toFixed(1).replace('.' , ',') + " " + metrica;
...
function runwayApp (event) {
    ...
    if (!windDirection || !runwayOne || !runwayTwo) {
        document.getElementById('insuficientData').innerHTML = "Fill in all fields!";
    ...
    if (!finalRunway || !finalRunway === NaN) {
        document.getElementById('insuficientData').innerHTML = "Insufficient data!";
        ...
        document.getElementById('rightRunway').innerHTML ="You will use runway " + finalRunway;
